...
std::size_t hybrid_bnn_2bit_simple::table_index(champsim::address ip)
{
...
auto hybrid_bnn_2bit_simple::base_predict(champsim::address ip) const -> base_prediction
{
...
void hybrid_bnn_2bit_simple::update_base(const base_prediction& prediction, bool taken)
{
...
bool hybrid_bnn_2bit_simple::predict_branch(champsim::address ip)
{
  ...
  // Always use BNN if base is weak
  const bool use_bnn = base.weak;
